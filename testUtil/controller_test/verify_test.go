...
func TestVerifyCode(t *testing.T)  {
	r := gin.Default()
	r.GET("/TestVerifyCode",VerifyCode)
	_ = r.Run(":8080")
}
...
func VerifyCode(c *gin.Context) {
	...
	//调用奖励接口，增加奖励
	result := AddGift(uid,code,gift)

	fmt.Printf("用户\"" + uid + "\"礼品码：\"" + code + "\"获取成功\n")
	c.String(http.StatusOK, "获取成功，获得礼品为："+result)
}

func VerifyGift(code string,uid string,gift *gift) string {
	//检查奖品码是否存在
...
func AddGift(uid string,code string,gift gift) string {
